...
- 正文最后一个非空行可以放空格分隔的 `#标签`。抖音、快手、B站保留在正文/简介底部；小红书会识别后逐个选择平台话题。上游应先根据内容和平台热度选好目标平台话题。
...
抖音竖封面上传后可能弹出“设置横封面获取更多流量”引导。流程必须按需处理：有横封面时点击“设置横封面”继续上传横封面，没有横封面时点击“暂不设置”；没弹窗则跳过，不能把它当成每次必有状态。
