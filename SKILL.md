...
- 小红书封面兼容新版语义输入与旧版弹窗；上传图必须重新加载，选择完整 3:4 和 100% 后点击“完成 / 确定”一次。编辑器正常关闭且主封面图片更新才继续；90 秒未完成则保留现场停止，禁止删除弹窗、取消保存或重复点击来伪造成功，上层仍须核对实际封面内容。
- 小红书 `xhs-publish-btn` 使用 CDP 穿透封闭 shadow root 定位主按钮，核对“发布 / 定时发布”与定时开关一致且无 disabled / loading；不点击含暂存按钮的宿主中心。目标不唯一或不可用就停止。
...
- B站封面编辑器的“首页推荐封面（4:3）”和“个人空间封面（16:9）”是两个独立画面。传入同一横版封面时，发布器依次点击两处画面，并各通过左下角上传入口上传一次；两处画布都更新后才保存封面和投稿。任一处未更新时保留编辑器并停止。
